...
cmd = "{0} norm -m- {1} | {0} view -i 'L10PVAL>{2}' | {0} query -f'%ID %L10PVAL\n' | awk 'BEGIN {{print \"SNP P\"}}; {{print $1, 10^-$2}}' | awk '!seen[$1]++' > {3}.tophits".format(args.bcftools_binary, args.bcf, -math.log10(args.clump_pval), args.out)
os.system(cmd)
